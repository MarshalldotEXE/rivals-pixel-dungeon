...
public class BlizzardBomb extends Bomb {
	...
	public void explode(int cell) {
		...
		for (int i : PathFinder.NEIGHBOURS9) {
			if (Dungeon.level.water[cell+i])
				GameScene.add(Blob.seed(cell+i, 12, Freezing.class));
			else
				GameScene.add(Blob.seed(cell+i, 10, Freezing.class));
		}
			...
			for (Item ingredient : ingredients){
				if (ingredient.quantity() > 0) {
					if (ingredient.getClass().equals(Bomb.class)) {
						bomb = true;
					} else if (ingredient instanceof PotionOfFrost
							|| ingredient instanceof PotionOfSnapFreeze) {
						main = true;
