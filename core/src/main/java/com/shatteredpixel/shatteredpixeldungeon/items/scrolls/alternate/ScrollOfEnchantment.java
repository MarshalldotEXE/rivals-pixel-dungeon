...
public class ScrollOfEnchantment extends AlternateInventoryScroll {
	
	{
		initials = 12;
		
		image = ItemSpriteSheet.EXOTIC_THURISAZ;
	...
	protected void onItemSelected( final Item item ) {
			...
			GameScene.show(new WndOptions(Messages.titleCase(ScrollOfEnchantment.this.name()),
				...
				protected void onSelect(int index) {
					if (index < 3) {
						...
						GLog.p(Messages.get(ScrollOfEnchantment.class, "weapon_glow"));
			...
			GameScene.show(new WndOptions(Messages.titleCase(ScrollOfEnchantment.this.name()),
				...
				protected void onSelect(int index) {
					if (index < 3) {
						...
						GLog.p(Messages.get(ScrollOfEnchantment.class, "armor_glow"));
						((ScrollOfEnchantment)curItem).readAnimation();
		...
		Statistics.upgradeUsed = true; //don't spawn naturally and can only be obtained via upgrade
	}
			...
			for (Item ingredient : ingredients){
				if (ingredient.quantity() > 0) {
					if (ingredient instanceof ScrollOfUpgrade
					 || ingredient instanceof ScrollOfMagicalInfusion) {
						main = true;
					} else if (ingredient instanceof MagicSandBag) {
						material = true;
					}
				}
			}
